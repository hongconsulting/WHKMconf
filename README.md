...
CI.R <- data.frame(WH_Rothman(s$time, s$surv, s$n.risk, s$n.event)[, 2:3])
CI <- cbind(CI, setNames(CI.R, c("Rl", "Ru")))
CI.TG <- data.frame(WH_ThomasGrunkemeier(s$time, s$n.risk, s$n.event)[, 2:3])
CI <- cbind(CI, setNames(CI.TG, c("TGl", "TGu")))
CI.N <- data.frame(WH_Nair(s$time, s$surv, s$std.err, s$n.risk, s$n.event)[, 2:3])
#> [WH_Nair] i = 0:11 of 11, lower = 0.0384615, upper = 0.537895
CI <- cbind(CI, setNames(CI.N, c("Nl", "Nu")))
CI.HM <- data.frame(WH_HollanderMcKeague(s$time, s$n.risk, s$n.event)[, 2:3])
...
3.  Hall, W.J. and Wellner, J.A., 1980. Confidence bands for a survival
...
5.  Klein, J.P. and Moeschberger, M.L., 2003. Appendix C: Statistical
...
8.  Sachs, M.C., Brand, A. and Gabriel, E.E., 2022. Confidence bands in
    survival analysis. *British Journal of Cancer*, 127(9),
    pp. 1636–1641.
9.  Thomas, D.R. and Grunkemeier, G.L., 1975. Confidence interval
